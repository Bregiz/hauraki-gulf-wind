...
Everything lives in **one file: `index.html`** — plain HTML + CSS + vanilla JS. **No build
step, no dependencies, no framework, no backend.** Keep it that way unless there's a strong
reason; simplicity is a feature here.
...
- **Responsive kiosk layout** filling the viewport: 4 columns (monitor) / 2 (tablet) /
  1 (phone).
- Verified working in-browser at multiple sizes and in both themes; no console errors.

It is **ready to deploy as-is** (static hosting — see README).
...
- **Base URL:** `https://api.zephyrapp.nz`
...
- **CORS is open** — the API reflects the request `Origin`, so the browser calls it directly.
  **This is why no backend/proxy is needed.** If you ever move to a source without open CORS,
  you'll need a small proxy.
...
- `fetchAll()` — `GET /stations`, returns a `{_id: station}` map.
...
- **History graphs**: `twoHourPoints()` (filter+convert+sort), `smoothLine()`
  ...
  (fetches all 8 `/stations/{id}/data` in parallel, every 5 min). The `.graph` div is an
...
- **Run:** double-click `index.html`, or `python3 -m http.server 8137` then open the URL.
- **Verify quickly:** the 8 tiles should populate within ~2 s; `fetch('https://api.zephyrapp.nz/stations')`
  in the console should return a large JSON array. No console errors expected.
- **Deploy:** any static host (Netlify drag-drop, Cloudflare Pages, Vercel, GitHub Pages).
  No build command. HTTPS required (the API is HTTPS).
